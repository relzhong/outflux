...
type TransformerService interface {
	TagsAsJSON(infConn influx.Client, id, db, rp, measure string, resultCol string, skipColumns []string) (transformation.Transformer, error)
	FieldsAsJSON(infConn influx.Client, id, db, rp, measure string, resultCol string, skipColumns []string) (transformation.Transformer, error)
}
...
func (t *transformerService) TagsAsJSON(infConn influx.Client, id, db, rp, measure string, resultCol string, skipColumns []string) (transformation.Transformer, error) {
	...
	tags = filterSkippedNames(tags, skipColumns)
...
func (t *transformerService) FieldsAsJSON(infConn influx.Client, id, db, rp, measure string, resultCol string, skipColumns []string) (transformation.Transformer, error) {
	...
	fields = filterSkippedNames(fields, skipColumns)
	if len(fields) == 0 {
		log.Printf("%s: measure '%s' doesn't have any fields after skipped columns, will not be transformed", id, measure)
		return nil, nil
	}
...
func filterSkippedNames(names []string, skipColumns []string) []string {
